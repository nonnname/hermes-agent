"""Platform-neutral helpers for smart mention routing."""
...
def default_smart_mention_system_prompt(platform_name: str) -> str:
    """Build the default classifier prompt for a messaging platform."""
    platform_name = str(platform_name or "group chat").strip() or "group chat"
    return f"""You are Hermes's {platform_name} smart mention router.

Decide whether the current {platform_name} group message is addressed to Hermes or asks Hermes to do something.
...
DEFAULT_SMART_MENTION_SYSTEM_PROMPT = default_smart_mention_system_prompt("Telegram")
DEFAULT_MATTERMOST_SMART_MENTION_SYSTEM_PROMPT = default_smart_mention_system_prompt(
    "Mattermost"
)
...
def normalize_smart_mention_config(
    raw: Any,
    *,
    default_system_prompt: str = DEFAULT_SMART_MENTION_SYSTEM_PROMPT,
) -> SmartMentionConfig:
    ...
    if not isinstance(raw, dict):
        return SmartMentionConfig(system_prompt=default_system_prompt)
...
def build_smart_mention_messages(
    *,
    ...
    user_parts = [
        f"{platform_name} group routing decision.",
        "",
        "Metadata:",
        "\n".join(metadata_lines),
        "",
...
def format_recent_context_for_agent(
    recent_context: Iterable[dict[str, Any]],
    max_chars: int,
    *,
    platform_name: str = "Telegram",
    ...
    if not body:
        return ""
    return (
        f"{platform_name} recent group context before the current smart-routed message:\n"
        f"{body}\n\n"
